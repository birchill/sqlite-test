...
type SQLableTypedArray = Uint8Array | Int8Array | ArrayBuffer;
...
declare global {
  ...
  function sqlite3InitModule(opts: InitOptions): Promise<Sqlite3Static>;
  ...
  type Sqlite3Static = {
    ...
    installOpfsSAHPoolVfs: (options: {
      /**
       * If truthy, contents and filename mapping are removed from each SAH it
       * is acquired during initalization of the VFS, leaving the VFS's storage
       * in a pristine state.
       *
       * Use this only for databases which need not survive a page reload.
       *
       * @default false
       */
      clearOnInit?: boolean;
      ...
      /**
       * Specifies the default capacity of the VFS.
       *
       * This should not be set unduly high because the VFS has to open (and
       * keep open) a file for each entry in the pool. This setting only has
       * an effect when the pool is initially empty. It does not have any effect
       * if a pool already exists.
       *
       * Note that this number needs to be at least twice the number of
       * expected database files (to account for journal files) and may need to
       * be even higher than three times the number of databases plus one,
      ...
      /**
       * Specifies the OPFS directory name in which to store metadata for the
       * VFS.
       *
       ...
       * invisible to each other. Changing this name will effectively orphan
       * any databases stored under previous names.
       *
       * This option may contain multiple path elements, e.g. "/foo/bar/baz",
       * and they are created automatically.
       *
       * In practice there should be no driving need to change this.
       *
       * WARNING: all files in this directory are assumed to be managed by the
       ...
       *
       * Normally this should not be changed, but it is possible to register
       * this VFS under multiple names so long as each has its own separate
       * directory to work from. The storage for each is invisible to all
       * others.
       *
       * The name must be a string compatible with sqlite3_vfs_register() and
       * friends and suitable for use in URI-style database file names.
       *
       * WARNING: if a custom name is provided, a custom directory must also be
       * provided if any other instance is registered with the default
       * directory. No two instances may use the same directory. If no
       * directory is explicitly provided then a directory name is synthesized
  ...
  class DB {
    constructor(options: { filename?: string; flags?: Flags; vfs?: any });
    constructor(filename?: string, flags?: Flags, vfs?: any);

    filename: string;
    pointer: number;

    // TODO: These can also take a WASM pointer but we don't have typings for
    // that yet.
    exec(
      sql: string | Array<string> | SQLableTypedArray,
      opts?: ExecThisOptions
    ): DB;
    exec(
      sql: string | Array<string> | SQLableTypedArray,
      opts?: ExecResultRowsOptions
    ): any;
    exec(
      sql: string | Array<string> | SQLableTypedArray,
      opts?: ExecSaveSqlOptions
    ): Array<unknown>;

    exec(sql: string | Array<string> | SQLableTypedArray): DB;
    exec(opts: ExecThisOptions): DB;
    exec(opts: ExecResultRowsOptions): any;
    exec(opts: ExecSaveSqlOptions): Array<unknown>;
  ...
  class OpfsSAHPoolUtil {
    ...
    reserveMinimumCapacity(minimumCapacity: number): Promise<number>;
    exportFile(name: string): Uint8Array;
    importDb(name: string, bytes: ArrayBuffer | Uint8Array): void;
    wipeFiles(): Promise<void>;
    unlink(filename: string): boolean;
    removeVfs(): Promise<boolean>;
    vfsName: string;
    OpfsSAHPoolDb: typeof DB;
  }
  ...
  type ExecSaveSqlOptions = ExecOptions & {
    returnValue: 'saveSql';
  };
